...
node .claude/skills/artifacts/scripts/artifact.mjs create page.html \
  --favicon 📊 --scope "app interaction flows" --watch "src/views/**"
```
...
| Versions | Every publish is an immutable version with an optional label. `?v=N` views history; `PUT` accepts `baseVersion` and returns 409 on conflicts (override with `force`). |
...
```
POST   /api/artifacts           { content, favicon, title?, description?, format?, label?, encrypted? }
                                → 201 { id, url, writeToken, version }
PUT    /api/artifacts/:id       same fields + baseVersion?/force?   (Bearer writeToken)
...
ciphertext as `content`. Max content size 4 MiB.
