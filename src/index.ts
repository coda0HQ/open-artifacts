...
// Self-hosted runtime-library bundle. The vendored mermaid ESM bundle lives in
// public/vendor/mermaid.bundle.mjs (regenerated by scripts/vendor-mermaid.mjs,
// which appends a side-effect line so `window.mermaid` is set when loaded as a
// module). Serving it same-origin — under script-src 'self' — lets mermaid
// load with NO external script host in the CSP, so the jsdelivr bypass (an
// artifact's inline JS createElement("script", {src: <external>}) to load an
// arbitrary package) is closed. The Worker re-serves the static asset with an
// explicit JavaScript MIME + nosniff so a module <script src> always loads,
// regardless of how the assets runtime types .mjs. Unlike /fonts, this is not
// opt-in: mermaid is a build-time-allowlisted library, so the bundle ships on
// every deploy and the route always serves it.
app.get("/vendor/mermaid.bundle.mjs", async (c) => {
  const asset = await c.env.ASSETS.fetch(
    new Request(`${new URL(c.req.url).origin}/vendor/mermaid.bundle.mjs`),
  );
  if (!asset.ok) {
    return new Response("not found", { status: 404 });
  }
