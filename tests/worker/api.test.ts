...
describe("the feedback channel is gone", () => {
  it("404s the removed feedback routes", async () => {
    const created = await createArtifact();
    ...
    const get = await exports.default.fetch(
      `${BASE}/api/artifacts/${created.id}/feedback`,
    );
    expect(get.status).toBe(404);
  });

  it("serves no feedback control in the host chrome", async () => {
    const created = await createArtifact();
    const html = await (
      await exports.default.fetch(`${BASE}/a/${created.id}`)
    ).text();
    expect(html).not.toContain("oa-feedback");
    // The comments control still ships — the surviving interaction channel.
