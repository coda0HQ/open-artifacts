...
authenticating once with `artifact login`, which stores a long-lived `sk_`
API key locally. The instance keeps only the key's hash, never the raw key.

## Give this to your coding agent

Copy this block to your agent to authenticate to coda0.com and obtain an API
key:
...
2. Run: artifact login --provider google
3. A browser opens to coda0.com - the user completes the Google sign-in there.
4. The API key (sk_) is stored automatically in .artifacts/credentials.json. You can then publish with `artifact create`.
```

## What `artifact login` does
...
org or with anyone who has the link. `artifact whoami` confirms who you are
logged in as; `artifact logout` clears the key.
